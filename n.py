                    #!/usr/bin/python3/attack/suraj-vip-bot/free/stop/STOPALL
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 2:
            ...
            try:
                duration = int(duration_str[:-1])  # Extract the numeric part of the duration
                if duration <= 0:
                    raise ValueError
                
                time_unit = duration_str[-1].lower()  # Extract the time unit (e.g., 'hour', 'day', 'week', 'month')
                if time_unit == 'h':
                    time_unit = 'hour'
                elif time_unit == 'd':
                    time_unit = 'day'
                ...
                if time_unit not in ('hour', 'day', 'week', 'month'):
                    raise ValueError

            except ValueError:
                response = "Invalid duration format. Please use '1h', '2d', '3w', or '4m' to add."
            ...
            if user_to_add not in allowed_user_ids:
                ...
                if set_approval_expiry_date(user_to_add, duration, time_unit):
                    response = f"User {user_to_add} has been successfully added. They need to provide feedback within {duration} {time_unit}s. Expiry date: {user_approval_expiry[user_to_add].strftime('%Y-%m-%d %H:%M:%S')}."
                else:
                    response = "Failed to properly add user. Please try again."
            else:
                response = "This user is already added."
        else:
            response = "Invalid command format. Use format like  'add [userid] 1h', '2d', '3w', '4m' to add."
    else:
...
Jisko khareedna h dm kro💸 :- @AEGON_THE_OWNER
YE Mera chennel h join kr lo sb  :- https://t.me/+nD2UGU3ANqs5ZWM1
'''
...
3. CHENNEL JOIN KR LO🔥 https://t.me/BGmMDDoS WARNA BOT OFF📴 HO JAYEGA
